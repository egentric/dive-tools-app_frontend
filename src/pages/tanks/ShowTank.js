...
const ShowTank = () => {
  ...
  const displayShowTank = async () => {
    try {
      ...
      setShowTank(response.data.data);
      setImage(response.data.data.qrcode_tank);
  ...
  // Vérifie si la date est expirée (plus de 1 an)
  const isDateExpired1 = (date) => {
    const oneYearInMilliseconds = 31536000000; // Nombre de millisecondes dans un an
    const currentDate = new Date(); // Date actuelle

    // Convertit la date du tiv en objet Date
    const tivDate = new Date(date);

    // Vérifie si la différence entre la date actuelle et la date du tiv dépasse un an
    return currentDate - tivDate > oneYearInMilliseconds;
  };

  const tivDate = formatDateShow(showTank.tiv_date);
  const isExpired1 = isDateExpired1(showTank.tiv_date);
  ...
  const requalifDate = formatDateShow(showTank.requalification_date);
  const isExpired6 = isDateExpired6(showTank.requalification_date);
  ...
  // console.log(isExpired6);
  // console.log(isTwoMonthsBeforeExpiration6(showTank.requalification_date));
  // console.log(requalifDate);
  // console.log(showTank.requalification_date);
  console.log(isExpired1);
  console.log(isTwoMonthsBeforeExpiration1(showTank.tiv_date));
  console.log(tivDate);
  // console.log(showTank.tiv_date);
  // console.log(isTwoMonthsBeforeExpiration6(showTank.requalification_date));
